# /opt/data/private/BlackBox/train.py train_t12-kl.py(添加kl散度)
...
from loss_kl import BlackBoxLoss
...
SAVE_DIR = os.path.join(ROOT, "save-kl0.0005", "demo")
FINAL_PATCH_DIR = os.path.join(ROOT, "save-kl0.0005", "final_patch")  # 新增：final patch专用目录
LOG_PATH = os.path.join(ROOT, "save-kl0.0005", "train.log")
VISUAL_DIR = os.path.join(ROOT, "save-kl0.0005", "visual")
...
def plot_training_curves(step_history, loss_history, grad_norm_history, lr_history, save_dir, dataloader):
    ...
    if len(valid_data[0]) >= 4:
        plt.plot(valid_steps, [l[3] for l in valid_data], label='Consistency Loss (KL)', color='#1f77b4', linestyle='--', linewidth=2)
    plt.xlabel('Global Step')
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for batch_idx, (imgs, _) in enumerate(dataloader):
            ...
            if batch_idx % 10 == 0:
                ...
                loss_history.append((total_loss.item(), det_loss_v, tv_loss_v, cons_loss_v))  # ✅ 增加一致性loss记录
