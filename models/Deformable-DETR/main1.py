...
def main(args):
    ...
    if args.eval:
        ...
        # === ✅ 导出检测结果（bbox + score + class） ===
        if coco_evaluator is not None:
            try:
                coco_eval_bbox = coco_evaluator.coco_eval.get("bbox", None)
                if coco_eval_bbox and coco_eval_bbox.cocoDt and hasattr(coco_eval_bbox.cocoDt, "dataset"):
                    detections = coco_eval_bbox.cocoDt.dataset  # list[dict]
                    print(f"Detected {len(detections)} objects, exporting...")

                    json_path = output_dir / "detections_bbox_with_score.json"
                    pth_path = output_dir / "detections_bbox_with_score.pth"
                    with open(json_path, "w", encoding="utf-8") as f:
                        json.dump(detections, f, indent=2)
                    torch.save(detections, pth_path)

                    print(f"✅ Saved detections:")
                    print(f"   JSON: {json_path}")
                    print(f"   PTH : {pth_path}")
                else:
                    print("⚠️ No cocoDt dataset found inside coco_evaluator.coco_eval['bbox'].")
    ...
    print(f"Training finished in {datetime.timedelta(seconds=int(time.time() - start_time))}")
