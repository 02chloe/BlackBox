...
# -------------------------------------
# 一致性蒸馏损失函数（KL散度）参数：beta: float = 1e-3 
# -------------------------------------
def compute_decoder_consistency_loss(
    ...
    if logits_all_layers is None or logits_all_layers.ndim != 4:
        return torch.tensor(0.0, device=logits_all_layers.device if logits_all_layers is not None else 'cpu')
    ...
    for k in range(L - 1):
        ...
        student_prob = torch.log_softmax(student_logits / tau, dim=-1)
        kl = F.kl_div(student_prob, teacher_prob, reduction='batchmean')
        total_kl += kl
        count += 1
...
class BlackBoxLoss:
    ...
    def __call__(
        ...
        return {
            ...
            'l2_loss': l2_loss,
            'nps_loss': nps_loss,
            'consistency_loss': consistency_loss    # ✅ 新增
        }
