# 文件: loss_kl_.py
# 目的：实现 BlackBox 论文风格的攻击损失（兼容 GSE + TMM + KL一致性）
...
#   tau:  softmax 温度
# -------------------------------------
def compute_decoder_consistency_loss(
    logits_all_layers: torch.Tensor,
    beta: float = 0.001,
    tau: float = 2.0
) -> torch.Tensor:
    """
    计算解码器各层与末层之间的 KL 一致性损失。

    参数:
        logits_all_layers: Tensor [L, B, Q, C]
        beta: float, 一致性损失的权重
        tau: float, softmax 温度（控制平滑程度）
    返回:
        consistency_loss: Tensor (scalar)
    """
    if logits_all_layers is None or logits_all_layers.ndim != 4:
        device = logits_all_layers.device if logits_all_layers is not None else "cpu"
        return torch.tensor(0.0, device=device)

    L, B, Q, C = logits_all_layers.shape
    if L < 2:
        return torch.tensor(0.0, device=logits_all_layers.device)

    # 最后一层作为教师
    teacher_logits = logits_all_layers[-1].detach()   # 不反传教师梯度
    teacher_prob = torch.softmax(teacher_logits / tau, dim=-1)

    total_kl = 0.0
    count = 0

    for k in range(L - 1):
        student_logits = logits_all_layers[k]
        student_log_prob = torch.log_softmax(student_logits / tau, dim=-1)
        # kl_div(input, target): input 为 log-prob, target 为 prob
        kl = F.kl_div(student_log_prob, teacher_prob, reduction='batchmean')
        total_kl += kl
        count += 1

    consistency_loss = beta * total_kl / max(1, count)
...
class BlackBoxLoss:
    ...
    主要组件（可配置）：
      ...
      - kl_beta: KL 一致性损失权重（外部可控）
      - kl_tau: KL 一致性 softmax 温度
    """
    ...
    def _compute_detection_loss_from_logits(
        ...
        if self.layer_aggregation == 'mean_logits':
            ...
            if self.use_sigmoid_for_binary and C == 1:
                ...
                loss_per_query = probs[..., 0] if probs.dim() == 2 else probs
            else:
    ...
    def __call__(
        self,
        imgs: torch.Tensor,              # 原始batch images [B,3,H,W]
        patched_imgs: torch.Tensor,      # patched images [B,3,H,W] or already NestedTensor
        patch_tensor: Optional[torch.Tensor] = None,  # 裁剪/原补丁 [1,C,ph,pw]
        ...
        logits_all = self.gse(samples, return_all_layers=True)  # [L, B, Q, C]
        logits_all = logits_all.to(self.device)

        # 2) detection loss
        det_loss = self._compute_detection_loss_from_logits(
            logits_all, target_class=self.target_class
        )

        # 2.5) decoder 一致性蒸馏 (KL 散度)，使用外部传入的 kl_beta 与 kl_tau
        consistency_loss = compute_decoder_consistency_loss(
            logits_all_layers=logits_all,
            beta=self.kl_beta,
            tau=self.kl_tau
        )
